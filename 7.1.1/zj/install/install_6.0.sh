...
Install_Pip(){
	...
	if [ -z "${isPip}" ];then
		wget -O get-pip.py ${cjqbt_Url}/src/get-pip.py
		python get-pip.py
		rm -f get-pip.py
		isPip=$(pip -V|grep python)
		if [ -z "${isPip}" ];then
...
Install_Pillow()
{
	...
	if [ "$isSetup" = "" ];then
		...
		wget -O Pillow-3.2.0.zip $cjqbt_Url/install/src/Pillow-3.2.0.zip -T 10
...
Install_psutil()
{
	...
	if [ "$isSetup" = "" ];then
		wget -O psutil-5.2.2.tar.gz $cjqbt_Url/install/src/psutil-5.2.2.tar.gz -T 10
...
Install_chardet()
{
	...
	if [ "${isSetup}" = "" ];then
		wget -O chardet-2.3.0.tar.gz $cjqbt_Url/install/src/chardet-2.3.0.tar.gz -T 10
