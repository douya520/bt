...
miui_Url='https://raw.githubusercontent.com/cq520/bt/master/7.1.1/zj';
...
{ 
	...
	if [ "$pyVersion" == 2 ];then
		...
		wget -O $pluginPath/btwaf_main.cpython-36m-x86_64-linux-gnu.zip $miui_Url/btwaf_rule/test/btwaf/btwaf_main.cpython-36m-x86_64-linux-gnu.zip -T 5
	...
	wget -O $pluginPath/firewalls_list.py $miui_Url/btwaf_rule/test/btwaf/firewalls_list.py -T 5
	wget -O $pluginPath/index.html $miui_Url/btwaf_rule/test/btwaf/index.html -T 5
	wget -O $pluginPath/info.json $miui_Url/btwaf_rule/test/btwaf/info.json -T 5
	wget -O $pluginPath/icon.png $miui_Url/btwaf_rule/test/btwaf/icon.png -T 5
	wget -O /www/server/panel/vhost/nginx/btwaf.conf $miui_Url/btwaf_rule/test/btwaf/btwaf.conf -T 5
	\cp -a -r /www/server/panel/plugin/btwaf/icon.png /www/server/panel/static/img/soft_ico/ico-btwaf.png
	wget -O $pluginPath/btwaf.zip $miui_Url/btwaf_rule/test/btwaf/btwaf.zip -T 5
...
Install_cjson()
{
	...
	if [ ! -f /usr/local/lib/lua/5.1/cjson.so ];then
		wget -O lua-cjson-2.1.0.tar.gz $miui_Url/btwaf_rule/test/btwaf/lua-cjson-2.1.0.tar.gz -T 20
...
{	
	...
	if [ ! -f /usr/local/lib/lua/5.1/socket/core.so ];then
		wget -O luasocket-master.zip $miui_Url/btwaf_rule/test/btwaf/luasocket-master.zip -T 20
