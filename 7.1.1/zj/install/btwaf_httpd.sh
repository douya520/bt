...
btsb_Url='https://raw.githubusercontent.com/cq520/bt/master/7.1.1/zj';
...
{	
	...
	if [ "$aacher" == "aarch64" ];then 
		...
		if [  -f /www/server/panel/pyenv/bin/python ];then
			...
			if [ "$pyVersion" == 2 ];then
				wget -O $pluginPath/btwaf_httpd_main.zip $btsb_Url/install/plugin/btwaf_httpd/btwaf_httpd_main.zip -T 5
				unzip -o $pluginPath/btwaf_httpd_main.zip -d $pluginPath > /dev/null
	...
	wget -O $pluginPath/firewalls_list.py $btsb_Url/install/plugin/btwaf_httpd/firewalls_list.py -T 5
	wget -O $pluginPath/index.html $btsb_Url/install/plugin/btwaf_httpd/index.html -T 5
	...
	rm -rf /www/server/panel/plugin/btwaf_httpd/btwaf_httpd_main.so
...
Install_cjson()
{
	...
	if [ "${Centos8Check}" ];then
		wget -O lua-5.3-cjson.tar.gz $download_Url/install/src/lua-5.3-cjson.tar.gz -T 20
