...
Install_total()
{
	...
	wget -O $pluginPath/total_main.py $cjqbt_Url/install/plugin/total/total_main.py -T 5
	wget -O $pluginPath/index.html $cjqbt_Url/install/plugin/total/index.html -T 5
	wget -O $pluginPath/info.json $cjqbt_Url/install/plugin/total/info.json -T 5
	wget -O $pluginPath/icon.png $cjqbt_Url/install/plugin/total/icon.png -T 5
	
	if [ ! -f /www/server/panel/static/js/china.js ];then
		wget -O /www/server/panel/static/js/china.js $cjqbt_Url/install/plugin/total/china.js -T 5
	fi
	wget -O /www/server/panel/vhost/apache/total.conf $cjqbt_Url/install/plugin/total/total_httpd.conf -T 5
	wget -O /www/server/panel/vhost/nginx/total.conf $cjqbt_Url/install/plugin/total/total_nginx.conf -T 5
	\cp -a -r /www/server/panel/plugin/total/icon.png /www/server/panel/static/img/soft_ico/ico-total.png
	wget -O $pluginPath/total.zip $cjqbt_Url/install/plugin/total/total.zip -T 5
	...
	if [ ! -f $total_path/config.json ];then
		wget -O $total_path/config.json $cjqbt_Url/install/plugin/total/config.json -T 5
	fi
...
Install_cjson()
{
	...
	if [ "${Centos8Check}" ];then
		...
		if [ ! -f /usr/lib/lua/5.3/cjson.so ];then
			wget -O lua-5.3-cjson.tar.gz $cjqbt_Url/src/lua-5.3-cjson.tar.gz -T 20
	...
	if [ ! -f /usr/local/lib/lua/5.1/cjson.so ];then
		wget -O lua-cjson-2.1.0.tar.gz $cjqbt_Url/install/src/lua-cjson-2.1.0.tar.gz -T 20
...
Install_socket()
{
	...
	if [ ! -f /usr/local/lib/lua/5.1/socket/core.so ];then
		wget -O luasocket-master.zip $cjqbt_Url/install/src/luasocket-master.zip -T 20
...
Install_mod_lua()
{
	...
	if [ ! -d /www/server/apache/src ];then
		wget -O httpd-2.4.33.tar.gz $cjqbt_Url/src/httpd-2.4.33.tar.gz -T 20
		tar xvf httpd-2.4.33.tar.gz
		rm -f httpd-2.4.33.tar.gz
		mv httpd-2.4.33 src
		cd /www/server/apache/src/srclib
		wget -O apr-1.6.3.tar.gz $cjqbt_Url/src/apr-1.6.3.tar.gz
		wget -O apr-util-1.6.1.tar.gz $cjqbt_Url/src/apr-util-1.6.1.tar.gz
